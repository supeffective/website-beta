...
type UseSearchStateReturnType<T extends NewSearchState> = [Partial<T>, (newState: Partial<T>) => void, (pathname: string, newState: Partial<T>) => {
    pathname: string;
...
export function useSearchState<T extends NewSearchState>(): UseSearchStateReturnType<T> {
    ...
    const buildHref = useCallback((pathname: string, newState: NewSearchState): {
        pathname: string;
        query: Record<string, string>;
    } => {
        const query = Object.fromEntries(Object.entries({ ...state, ...newState })
    ...
    const updateState = useCallback((newState: NewSearchState): void => {
